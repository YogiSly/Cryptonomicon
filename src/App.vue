          ...
          <div class="max-w-xs">
            ...
            <div class="mt-1 relative rounded-md shadow-md">
              <input v-model="ticker" @keydown.enter="add" type="text" name="wallet" id="wallet"
                ...
                placeholder="Например DOGE" @input="findTicker" />
            </div>

            <div v-if="showListTicker" class="flex bg-white shadow-md p-1 rounded-md shadow-md flex-wrap">
              <span v-for="item in this.fiterTicker" :key="item" @click="addTickerInFilter(item)"
                class="inline-flex items-center px-2 m-1 rounded-md text-xs font-medium bg-gray-300 text-gray-800 cursor-pointer">
                {{  item  }}
              </span>

            </div>
            <div v-if="showErrorTicker" class="text-sm text-red-600">Такой тикер уже добавлен</div>
...
export default {
  ...
  data() {
    return {
      ...
      arrayCoins: [],
      fiterTicker: [],
      showListTicker: false,
      showErrorTicker: false,
      filter: '',
      page: 1,
  ...
  methods: {
    ...
    addTickerInFilter(ticker) {
      ...
      if (this.tickers.length > 0) {
        const ticketRes = this.tickers.find(item => item.name === ticker)
    ...
    findTicker(e) {
      ...
      for (let i = 0; i < this.arrayCoins.length; i++) {
        const element = this.arrayCoins[i];
